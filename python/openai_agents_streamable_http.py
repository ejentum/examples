...
with bearer auth. The agent sees all four harness tools at once and picks
the one that fits each user question.

This is the MCP-transport variant of openai_agents_tool.py (which uses
function tools over the REST gateway). Use this when you want to wire the
harness in as an MCP server rather than as inline Python tools, or when
you are already running an Agents SDK app that uses MCP servers for
other tools.
...
Env:
    OPENAI_API_KEY=sk-...
    EJENTUM_API_KEY=ek-...        # https://ejentum.com/dashboard
"""
...
async def main() -> None:
    ...
    async with MCPServerStreamableHttp(
        ...
        with trace(workflow_name="ejentum-streamable-http", trace_id=trace_id):
